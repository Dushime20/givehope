...
const VideoSection = ({ title, videos }) => (
  <section className="py-16 px-4">
    <div className="max-w-7xl mx-auto">
      <h2 className="text-3xl md:text-4xl font-bold text-gray-800 mb-10 text-left border-l-4 border-yellow-400 pl-4">
        {title}
      </h2>
      <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3 justify-items-center">
        {videos.map(({ videoId, title, description }) => (
          <VideoCard key={videoId} videoId={videoId} title={title} description={description} />
        ))}
      </div>
    </div>
  </section>
...
const Videos = () => {
  return (
    <div className="bg-white">
      {/* Header */}
      <div className="bg-yellow-100 py-16 text-center px-4">
        <h1 className="text-4xl md:text-5xl font-bold text-gray-800 mb-4">Our Inspiring Stories</h1>
        <p className="text-gray-700 max-w-2xl mx-auto text-lg">
          Discover real stories and impactful moments through our curated video collection. Watch, learn, and be inspired.
        </p>
      </div>
