...
const EditSuggestion = () => {
  const { id } = useParams();
  // In a real app, fetch suggestion by id. Here, use mockSuggestion.
  const [text, setText] = useState(mockSuggestion.text);
  const [user, setUser] = useState(mockSuggestion.user);
  const [status, setStatus] = useState(mockSuggestion.status);
  ...
  return (
    <div className="max-w-lg mx-auto p-6 bg-white rounded shadow-md mt-8">
      ...
      {success && (
        <div className="mb-4 p-2 bg-green-100 text-green-700 rounded">Suggestion updated successfully!</div>
      )}
